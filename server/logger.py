...
class LoggerGlobals:
    ...
    LOGS_PATH = Path(__file__).parent / '.dispatcher'
...
def log_file():
    return os.path.expanduser(os.path.join(logger_globals.LOGS_PATH, 'faraday-dispatcher.log'))


MAX_LOG_FILE_SIZE = 5 * 1024 * 1024     # 5 MB
MAX_LOG_FILE_BACKUP_COUNT = 5
ROOT_LOGGER = u'pycon2020_server'
LOGGING_HANDLERS = []
...
def setup_console_logging(formatter):
    ...
    console_handler.setLevel(logger_globals.LOGGING_LEVEL)
...
def get_logger(obj=None):
    ...
    if obj is None:
        logger = logging.getLogger(ROOT_LOGGER)
        logger.setLevel(logger_globals.LOGGING_LEVEL)
...
def set_logging_level(level):
    ...
    logger_globals.LOGGING_LEVEL = level
...
def reset_logger(logger_folder=None):
    if logger_folder is not None:
        ...
        logger_globals.LOGS_PATH = logger_folder
    setup_logging()
